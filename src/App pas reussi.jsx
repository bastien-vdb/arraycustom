  ...
  const handleSearch = () => {
    fetch(`${api.base}q=${search}`)
      ...
      .then((result) => {
        ...
        fetch(
          `${url.base}latitude=${result[0].lat}&longitude=${result[0].lon}&start_date=2013-01-01&end_date=2013-02-28&daily=temperature_2m_max,temperature_2m_min,rain_sum&timezone=auto`
        )
          .then((res1) => res1.json())
          .then((resultat) => {
            ...
            // console.log(resultat.daily.time.some((date) => date.startsWith("2022-01")));
                // resultat.daily(time=>)

                const arrayJanv  = result.daily.time.filter((element) => element.time.startsWith("2022-01"));
              ...
              // meteoDataForYearCalculated[0] = dataOfOneMonth;
